...
public class BookDao {
	...
	public List<Book> bookRankingList() throws SQLException {
		String sql = "SELECT * from book " + "JOIN orderitem ON orderitem.bid=book.bid "
				+"JOIN `order` ON `order`.oid=orderitem.oid " + "WHERE `order`.`status`=4 "
				+"GROUP BY book.bid " + "order BY SUM(quantity) desc limit 6";
