...
public class OrderServlet extends MyServlet {
	...
	public String payment(HttpServletRequest req, HttpServletResponse resp) throws SQLException {
		...
		if(password.equals(ps)) {
			...
			req.setAttribute("code", "error");
			req.setAttribute("msg", "密码错误，支付失败");
			return "f:/frontpages/msg1.jsp";
		}
	}
	...
	public String myOrders(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		int pc = getPc(req);//1.获取当前页码
		String url = getUrl(req);
		User user = (User)req.getSession().getAttribute("sessionUser");//2.获取当前用户uid
		PageBean<Order> pb = orderService.myOrders(user.getUid(), pc);
		pb.setUrl(url);//3.获取url，设置给PageBean
		req.setAttribute("pb", pb);//4.把PageBean保存到request，转发到/jsps/order/list.jsp
